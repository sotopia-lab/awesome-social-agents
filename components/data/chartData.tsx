export const bar_data = [
   {
  name: 'Text',
  overall: 164,
  collaboration: 24,
  competition: 9,
  mixed_objectives: 56,
  text: 164,
  embodied: 7,
  virtual: 24,
  robotics: 5,
  two_agents: 58,
  reinforcement_learning: 21,
  agents_with_personas: 25,
  human: 93,
  not_applicable: 148,
  rule_based: 58,
  more_than_three_agents: 36,
  more_information_asymmetrical: 2,
  prompting_and_in_context_learning: 48,
  qualitative: 35,
  human_agent: 56,
  implicit_objectives: 18,
  finetuning: 36,
  agents_with_memory: 16,
  more_omniscient: 7,
  pretraining: 18,
  model_based: 48,
  simulated_humans: 15,
  agent_teams: 6,
  health: 18,
  education: 8,
  policy: 12,
},
{
  name: 'Virtual',
  overall: 96,
  virtual: 96,
  prompting_and_in_context_learning: 43,
  rule_based: 89,
  not_applicable: 52,
  mixed_objectives: 41,
  two_agents: 37,
  qualitative: 22,
  human: 45,
  human_agent: 36,
  more_omniscient: 27,
  simulated_humans: 26,
  finetuning: 22,
  embodied: 4,
  reinforcement_learning: 17,
  implicit_objectives: 13,
  agents_with_memory: 10,
  pretraining: 1,
  collaboration: 14,
  competition: 6,
  agent_teams: 9,
  more_than_three_agents: 17,
  fully_omniscient: 1,
  model_based: 5,
  health: 3,
  education: 25,
  robotics: 1,
  agents_with_personas: 1,
},
{
  name: 'Embodied',
  overall: 58,
  collaboration: 32,
  embodied: 58,
  prompting_and_in_context_learning: 16,
  more_than_three_agents: 10,
  rule_based: 48,
  not_applicable: 39,
  finetuning: 17,
  qualitative: 10,
  human_agent: 21,
  mixed_objectives: 13,
  reinforcement_learning: 19,
  simulated_humans: 7,
  robotics: 11,
  fully_omniscient: 2,
  pretraining: 2,
  two_agents: 22,
  human: 16,
  health: 1,
  agent_teams: 5,
  agents_with_memory: 5,
  implicit_objectives: 5,
  competition: 5,
  model_based: 4,
  education: 1,
  agents_with_personas: 4,
},
{
  name: 'Robotics',
  overall: 60,
  robotics: 60,
  prompting_and_in_context_learning: 7,
  pretraining: 1,
  human: 48,
  qualitative: 19,
  not_applicable: 28,
  human_agent: 38,
  collaboration: 20,
  mixed_objectives: 21,
  two_agents: 32,
  finetuning: 6,
  reinforcement_learning: 17,
  agents_with_memory: 4,
  health: 7,
  implicit_objectives: 7,
  rule_based: 25,
  simulated_humans: 7,
  agent_teams: 5,
  agents_with_personas: 1,
  more_than_three_agents: 5,
  model_based: 7,
  fully_omniscient: 1,
  competition: 2,
  education: 5,
  more_omniscient: 3,
},
];
export const area_data = [
   {
  name: '2017',
  text: 2,
  mixed_objectives: 1,
  reinforcement_learning: 4,
  two_agents: 2,
  agents_with_memory: 2,
  rule_based: 5,
  human_agent: 3,
  virtual: 3,
  not_applicable: 4,
  competition: 1,
  robotics: 1,
  qualitative: 1,
  human: 1,
  more_omniscient: 0,
  prompting_and_in_context_learning: 0,
  more_than_three_agents: 0,
  fully_omniscient: 0,
  finetuning: 0,
  health: 0,
  model_based: 0,
  agents_with_personas: 0,
  education: 0,
  more_information_asymmetrical: 0,
  simulated_humans: 0,
  policy: 0,
  collaboration: 0,
  agent_teams: 0,
  embodied: 0,
  implicit_objectives: 0,
  pretraining: 0,
},
{
  name: '2018',
  collaboration: 3,
  agents_with_personas: 2,
  two_agents: 10,
  qualitative: 5,
  human: 8,
  not_applicable: 13,
  text: 6,
  mixed_objectives: 5,
  finetuning: 4,
  reinforcement_learning: 4,
  agents_with_memory: 1,
  implicit_objectives: 2,
  human_agent: 5,
  virtual: 6,
  embodied: 2,
  rule_based: 9,
  prompting_and_in_context_learning: 1,
  robotics: 3,
  simulated_humans: 1,
  fully_omniscient: 1,
  more_than_three_agents: 3,
  competition: 1,
  health: 2,
  model_based: 1,
  education: 1,
  more_omniscient: 1,
  more_information_asymmetrical: 0,
  policy: 0,
  agent_teams: 0,
  pretraining: 0,
},
{
  name: '2019',
  collaboration: 8,
  competition: 3,
  mixed_objectives: 5,
  text: 7,
  two_agents: 8,
  more_than_three_agents: 4,
  reinforcement_learning: 6,
  rule_based: 11,
  not_applicable: 11,
  finetuning: 3,
  human: 9,
  human_agent: 6,
  implicit_objectives: 2,
  agents_with_personas: 2,
  qualitative: 5,
  virtual: 4,
  prompting_and_in_context_learning: 1,
  embodied: 6,
  robotics: 2,
  agent_teams: 1,
  model_based: 1,
  health: 1,
  more_omniscient: 0,
  fully_omniscient: 0,
  education: 0,
  more_information_asymmetrical: 0,
  simulated_humans: 0,
  agents_with_memory: 0,
  policy: 0,
  pretraining: 0,
},
{
  name: '2020',
  text: 18,
  mixed_objectives: 9,
  more_than_three_agents: 6,
  model_based: 11,
  human_agent: 11,
  collaboration: 5,
  rule_based: 16,
  not_applicable: 25,
  finetuning: 6,
  agents_with_personas: 1,
  qualitative: 5,
  human: 18,
  embodied: 4,
  reinforcement_learning: 6,
  simulated_humans: 3,
  virtual: 9,
  prompting_and_in_context_learning: 1,
  robotics: 5,
  fully_omniscient: 1,
  two_agents: 9,
  pretraining: 4,
  competition: 1,
  implicit_objectives: 1,
  health: 2,
  policy: 1,
  education: 2,
  more_omniscient: 2,
  more_information_asymmetrical: 0,
  agents_with_memory: 0,
  agent_teams: 0,
},
{
  name: '2021',
  virtual: 7,
  reinforcement_learning: 8,
  rule_based: 17,
  not_applicable: 31,
  embodied: 10,
  finetuning: 6,
  collaboration: 9,
  mixed_objectives: 12,
  robotics: 9,
  two_agents: 18,
  human: 19,
  human_agent: 15,
  simulated_humans: 2,
  text: 19,
  model_based: 6,
  competition: 1,
  more_than_three_agents: 5,
  qualitative: 8,
  implicit_objectives: 1,
  health: 3,
  agent_teams: 1,
  policy: 3,
  education: 1,
  more_omniscient: 1,
  agents_with_personas: 1,
  prompting_and_in_context_learning: 1,
  fully_omniscient: 0,
  more_information_asymmetrical: 0,
  agents_with_memory: 0,
  pretraining: 0,
},
{
  name: '2022',
  text: 20,
  implicit_objectives: 9,
  more_than_three_agents: 8,
  pretraining: 3,
  finetuning: 18,
  human: 30,
  rule_based: 29,
  human_agent: 21,
  competition: 3,
  reinforcement_learning: 11,
  mixed_objectives: 16,
  prompting_and_in_context_learning: 6,
  agents_with_memory: 5,
  qualitative: 14,
  not_applicable: 34,
  agents_with_personas: 5,
  collaboration: 15,
  two_agents: 27,
  virtual: 14,
  embodied: 13,
  robotics: 10,
  agent_teams: 3,
  health: 4,
  model_based: 4,
  simulated_humans: 7,
  education: 6,
  policy: 2,
  more_omniscient: 2,
  fully_omniscient: 0,
  more_information_asymmetrical: 0,
},
{
  name: '2023',
  collaboration: 25,
  embodied: 25,
  prompting_and_in_context_learning: 55,
  more_than_three_agents: 21,
  rule_based: 68,
  not_applicable: 81,
  text: 53,
  implicit_objectives: 18,
  finetuning: 22,
  human: 53,
  human_agent: 42,
  reinforcement_learning: 18,
  competition: 6,
  two_agents: 40,
  mixed_objectives: 37,
  agents_with_memory: 17,
  more_omniscient: 11,
  qualitative: 23,
  simulated_humans: 25,
  virtual: 31,
  robotics: 23,
  pretraining: 7,
  agent_teams: 10,
  agents_with_personas: 8,
  model_based: 18,
  fully_omniscient: 1,
  health: 7,
  policy: 2,
  education: 8,
  more_information_asymmetrical: 0,
},
{
  name: '2024',
  collaboration: 13,
  competition: 4,
  mixed_objectives: 30,
  text: 34,
  embodied: 7,
  virtual: 35,
  robotics: 10,
  two_agents: 25,
  reinforcement_learning: 7,
  agents_with_personas: 5,
  human: 38,
  not_applicable: 50,
  rule_based: 46,
  more_than_three_agents: 14,
  more_information_asymmetrical: 2,
  prompting_and_in_context_learning: 46,
  qualitative: 17,
  human_agent: 26,
  finetuning: 19,
  more_omniscient: 12,
  pretraining: 8,
  simulated_humans: 13,
  fully_omniscient: 1,
  agents_with_memory: 8,
  health: 7,
  agent_teams: 8,
  implicit_objectives: 8,
  model_based: 19,
  education: 12,
  policy: 3,
},
{
  name: '2025',
  text: 1,
  more_than_three_agents: 1,
  agents_with_personas: 1,
  prompting_and_in_context_learning: 1,
  model_based: 1,
  human: 1,
  simulated_humans: 1,
  policy: 1,
  not_applicable: 0,
  more_omniscient: 0,
  qualitative: 0,
  virtual: 0,
  fully_omniscient: 0,
  finetuning: 0,
  health: 0,
  competition: 0,
  rule_based: 0,
  human_agent: 0,
  two_agents: 0,
  education: 0,
  more_information_asymmetrical: 0,
  reinforcement_learning: 0,
  mixed_objectives: 0,
  agents_with_memory: 0,
  collaboration: 0,
  agent_teams: 0,
  robotics: 0,
  embodied: 0,
  implicit_objectives: 0,
  pretraining: 0,
},
{
  name: '2026',
  implicit_objectives: 1,
  virtual: 1,
  finetuning: 1,
  prompting_and_in_context_learning: 1,
  more_than_three_agents: 1,
  agents_with_memory: 1,
  human: 1,
  model_based: 1,
  human_agent: 1,
  more_omniscient: 1,
  not_applicable: 0,
  qualitative: 0,
  fully_omniscient: 0,
  health: 0,
  competition: 0,
  rule_based: 0,
  agents_with_personas: 0,
  two_agents: 0,
  education: 0,
  more_information_asymmetrical: 0,
  reinforcement_learning: 0,
  text: 0,
  simulated_humans: 0,
  mixed_objectives: 0,
  policy: 0,
  collaboration: 0,
  agent_teams: 0,
  robotics: 0,
  embodied: 0,
  pretraining: 0,
},
];
